	...
	<BasePage>
		<div class="inventory-outbound">
			...
			<el-table :data="tableData" border style="width: 100%;" :row-class-name="rowClass">
				...
				<el-table-column min-width="180" label="品牌 / 分类 / 仓位" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						<div><span>品牌：</span> <b>{{ scope.row.brandName }}</b> </div>
						<div><span>分类：</span> <b>{{ scope.row.goodsName }}</b> </div>
						<div><span>仓位：</span> <b>{{ scope.row.repositoryNames }}</b> </div>
					</template>
				</el-table-column>
				<el-table-column width="260" label="型号 / 库存数量" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						<div><span>型号：</span> <b>{{ scope.row.model }}</b> </div>
						<div><span>数量：</span> <b>{{ scope.row.num }}</b> </div>
						<!-- <div><span>单价：</span> <b>{{ scope.row.price }}</b> </div> -->
					</template>
				</el-table-column>
				<el-table-column width="260" label="出库数量 / 价格" :align="config.align" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						<div>
							<el-input type="number" style="width: 100%;" placeholder="请选择出库数量" v-model="scope.row.outnum" @change="val => {
						...
						<div style="margin-top: 5px;">
							<el-input type="number" style="width: 100%;" placeholder="请选择出库价格" v-model="scope.row.outprice">
	...
	export default {
		...
		computed: {
			money() {
				...
				return this.tableData.map(item => item.outnum * item.outprice).reduce((a, b) => a + b) || 0;
			}
		},
		methods: {
			querySearchAsync(queryString, cb){
				this.$api.stock.outSearch(queryString, false).then(res => {
					cb(res.data.map(item => {
						return {
							...item,
							value: [item.brandName, item.goodsName, item.repositoryNames, item.num].join(' , ')
						}
					}))
				});
			},
			...
			save(){
				this.$api.stock.outSave({
					...this.form,
					goodsList: this.tableData.map(item => ({...item, num: item.outnum, price: item.outprice}))
				}).then(res => {
					this.$router.push({
						name: 'inventory-out',
						params: {
