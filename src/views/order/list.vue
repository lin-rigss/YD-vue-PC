	...
	<BasePage>
		<div class="order-list">
			...
			<!-- <div class="order-status-tab">
				<el-button-group>
					<el-button 
						v-for="(btn, index) in $g.dic.orderStatus" :key="index" :type="formSea.status === btn.value ? 'primary':''"
						@click="changeStatus(btn)"
					>{{ btn.name }}</el-button>
				</el-button-group>
				
				<el-button class="tips" type="text" size="mini">订单类型简介及管理方法</el-button>
			</div> -->
			...
			>
				...
				<el-table-column prop="status" label="订单状态" width="120" :align="config.align" >
				...
				<el-table-column label="订单操作" min-width="80" fixed="right" :align="config.align" >
					<template slot-scope="scope">
						<el-button v-if="scope.row.status != '8'&&scope.row.status != '1'&&scope.row.status != '2'" type="text" @click="toDetail(scope.row)">订单详情</el-button>
						<!-- <el-button v-if="scope.row.status == '8'" type="text" @click="toInventory(scope.row)">入库</el-button> -->
						<el-button v-if="scope.row.status == '2'" type="text" @click="toOrder(scope.row)">去报价</el-button>
