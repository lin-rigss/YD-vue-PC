	...
	<BasePage>
		<div class="spare-list">
			...
			<div class="aside">
				...
				<el-button type="primary" size="mini" @click="toCreate">新增拆解产品</el-button>
				<div class="search">
			...
			<el-table :data="tableData" style="width: 100%" :size="config.size" :height="tableHeight" border>
				...
				<el-table-column width="120" label="二级分类编号" :align="config.align" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						{{ scope.row.code }}
					</template>
				</el-table-column>
				<el-table-column width="120" label="二级分类名称" :align="config.align" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						{{ scope.row.nameZn }} ( {{ scope.row.nameEn }} )
					</template>
				</el-table-column>
				<el-table-column width="120" label="零件编号" :align="config.align" :show-overflow-tooltip="config.hidden">
					<template slot-scope="scope">
						{{ scope.row.code }}
					</template>
				</el-table-column>
			...
			<el-dialog v-if="dialog.show" :title="dialog.title" :visible="dialog.show" append-to-body @close="() => {
				...
				<el-form :model="dialog.info" ref="form" :rules="$g.rules.parts">
					<el-form-item prop="unit" label="二级分类" :label-width="dialog.width">
						<el-select v-model="dialog.info.unit" placeholder="请选择二级分类" style="width: 100%;">
					...
					<el-form-item prop="nameZn" label="拆解产生产品名称" :label-width="dialog.width">
						<el-input v-model="dialog.info.nameZn" placeholder="请输入拆解产生产品名称" autocomplete="off"></el-input>
					</el-form-item>
